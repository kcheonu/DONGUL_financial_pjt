...
def index(request):
    try:
        ...
        # 오늘 날짜 계산
        today = datetime.today().strftime('%Y%m%d')  # 날짜 포맷: YYYYMMDD

        # API URL 생성
        api_url = f"{BASE_API_URL}?authkey={settings.EXCHANGE_API_KEY}&data={data_type}&searchdate={today}"
        ...
        data = response.json()
        if not data:
            print("API에서 데이터를 반환하지 않았습니다.")
            return Response({'message': '오늘 데이터가 없습니다.'}, status=status.HTTP_204_NO_CONTENT)
        ...
        for entry in processed_data:
            # 중복 확인: 동일한 `cur_unit`과 `deal_bas_r` 데이터가 이미 존재하는지 확인
        ...
        return Response({'message': '오늘 데이터를 성공적으로 저장했습니다.'}, status=status.HTTP_200_OK)
...
@api_view(['GET'])
def fetch_last_year_data(request):
    try:
        # 요청에서 데이터 타입 가져오기 (기본값 'AP01')
        data_type = request.GET.get('data', 'AP01')

        # 오늘 날짜 기준 최근 1년간의 날짜 계산
        ...
        # 1년간의 데이터를 일별로 수집
        for single_date in (one_year_ago + timedelta(days=n) for n in range(366)):
            search_date = single_date.strftime('%Y%m%d')  # 날짜 포맷: YYYYMMDD

            # API URL 생성
            api_url = f"{BASE_API_URL}?authkey={settings.EXCHANGE_API_KEY}&data={data_type}&searchdate={search_date}"
            ...
            if response.status_code == 200:
                ...
                print(f"날짜 {search_date} 데이터 요청 실패. 상태 코드: {response.status_code}")
        ...
        # 데이터 직렬화 및 저장
        serializer = ExchangeSerializer(data=processed_data, many=True)
        if not serializer.is_valid():
            print(f"직렬화 오류: {serializer.errors}")
            return Response({'error': serializer.errors}, status=status.HTTP_400_BAD_REQUEST)

        serializer.save()
        return Response({'message': '최근 1년간 데이터가 성공적으로 저장되었습니다.'}, status=status.HTTP_200_OK)
